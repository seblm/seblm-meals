...
class StatsSuite extends CatsEffectSuite:
  ...
  test("all meals for monday dinner"):
    ...
    val session: Resource[IO, Session[IO]] =
      Session.Builder[IO].withHost(host).withDatabase(db).withUserAndPassword(user, password).single
    ...
    val result: IO[List[String]] = session.use: session =>
      for
        ...
        reversedIntOrdering = summon[Ordering[Int]].reverse
        byOccurrence = terms
          .groupBy(identity)
          .view
          .mapValues(_.length)
          .toMap
          .foldLeft(SortedMap[Int, SortedSet[String]]()(using reversedIntOrdering)): (map, termAndCount) =>
