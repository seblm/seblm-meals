...
class MealsController(cc: ControllerComponents, mealsService: MealsService) extends AbstractController(cc) with Logging:
  ...
  private given mealSuggestWrites: Writes[MealSuggest] = { case mealSuggest: MealSuggest =>
    Json.obj(
  ...
  private given Writes[SuggestResponse] = { case suggestResponse: SuggestResponse =>
